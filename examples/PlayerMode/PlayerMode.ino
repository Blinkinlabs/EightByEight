...
const unsigned builtin_demo_count = sizeof(builtin_demos)/sizeof(Demo *);
unsigned demo_num = 0;
Demo * demo;
...
void load_demo(unsigned new_demo_num)
{
    ...
    unsigned int total_demos = patterns.getCount() + builtin_demo_count;

    Serial.print("demo count:");
    Serial.println(total_demos);
...
//    }
    ...
    if(demo != nullptr)
      demo->step(badge.ax, badge.ay, badge.az);
    ...
    if (!streaming)
    {
        ...
        if(demo != nullptr) {
          ...
          // Clear the display, but draw a blue corner
          badge.matrix.clear();
          badge.matrix.set(0,0,0,0,255);
          badge.matrix.set(7,0,255,0,0);
          badge.matrix.set(0,7,0,255,0);
          badge.matrix.set(7,7,255,255,255);
        }
